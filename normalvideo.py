...
# Open the video file (or use a webcam, here using video file)
cap = cv2.VideoCapture("fall.mp4")

# Get the video frame width, height, and frames per second (FPS)
frame_width = 1020  # Matching the resized width
frame_height = 500  # Matching the resized height
fps = int(cap.get(cv2.CAP_PROP_FPS))

# Define the codec and create a VideoWriter object to save the output video
fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # Codec for MP4
out = cv2.VideoWriter('output_fall_detection.mp4', fourcc, fps, (frame_width, frame_height))

if not out.isOpened():
    print("Error: Could not open video writer")

count = 0
blurred_track_ids = set()  # To store track_ids of objects being blurred
        ...
        if masks is not None:
            ...
            for box, track_id, class_id, mask in zip(boxes, track_ids, class_ids, masks):
                ...
                if thresh <= 0:
                    ...
                    # Track this object as blurred
                    blurred_track_ids.add(track_id)

                else:
                    # If the object was previously blurred, remove it from the set
                    if track_id in blurred_track_ids:
                        blurred_track_ids.remove(track_id)
